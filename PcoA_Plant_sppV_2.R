...
df<-merge(df,treatments)
df<-df[df$water==3,-c(4:5)]
...
n_vec<-treatments[treatments$water==3,2]

colvec_nitrogen <- c(wes_palette("Darjeeling1")[3], wes_palette("Darjeeling1")[5], wes_palette("Darjeeling1")[3],
...
png("figures/Plant_Nitrogen_only_unfertilized.png",res = 300, pointsize = 12,height=5.5,width=8,units="in")
ordiplot(pcoa.res, type = "none", las = 1, main = "PLANT - PCoA with Bray-Curtis - Nitrogen (95%)", xlab = "MDS1 (34%)", ylab = "MDS2 (22%)")
mtext(text ="ALL WATER TREATMENTS",side = 3)
#Ellipses are put onto the image before points, so points sit on top of confidence intervals and are more visable
ordiellipse(pcoa.res, nitrogen, draw="polygon", border = NA, kind = "sd", conf = 0.95, label = FALSE, col = c(wes_palette("Darjeeling1")[3],wes_palette("Darjeeling1")[5]), lty = c(2))

points(pcoa.res, display = "sites", col = colvec_nitrogen, pch = 20, cex = 2)

legend(x="bottomleft", pch = c(16, 16, 16), col = c(wes_palette("Darjeeling1")[3], wes_palette("Darjeeling1")[5]), legend = c("No Nitrogen", "Nitrogen"), bty = "n")
dev.off()
...
water_vec<-treatments[treatments$N==0,3]
#Site x Species matrix; now begin playing in Vegan
colvec_water <- c(wes_palette("Darjeeling1")[3], wes_palette("Darjeeling1")[2], wes_palette("Darjeeling1")[3], 
                  wes_palette("Darjeeling1")[2], wes_palette("Darjeeling1")[5], wes_palette("Darjeeling1")[5], 
                  wes_palette("Darjeeling1")[3], wes_palette("Darjeeling1")[2], wes_palette("Darjeeling1")[3], 
                  wes_palette("Darjeeling1")[5], wes_palette("Darjeeling1")[2], wes_palette("Darjeeling1")[5],
                  wes_palette("Darjeeling1")[2], wes_palette("Darjeeling1")[2], wes_palette("Darjeeling1")[3], 
...
summary(pcoa.res)
png("figures/Plant_Water_only_unfertilized.png",res = 300, pointsize = 12,height=5.5,width=8,units="in")
ordiplot(pcoa.res, type = "none", las = 1, main = "PLANT - PCoA with Bray-Curtis - Water (95%)", xlab = "MDS1 (31%)", ylab = "MDS2 (22%)")
